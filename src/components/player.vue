<template>
  ...
  <div
    class="player"
    :s_id="currentSong.s_id"
    v-if="playerOpen && currentSong"
    ref="player"
  >
    <div class="player_left">
      <img
        @click="showModal"
        class="screen"
        :src="`${publicPath}image/icon/screen.svg`"
        alt="放大視窗"
      />
      <img
        class="musicPic"
      ...
      >
        <source
          :src="`${publicPath}audio/${currentSong.s_src}`"
          type="audio/mpeg"
        />
      </audio>
    </div>
...
export default {
  ...
  methods: {
    ...
    prevSong() {
      ...
      const nextIndex =
        (currentIndex - 1 + this.songList.length) % this.songList.length;
      const nextSId = this.songList[nextIndex].s_id;
