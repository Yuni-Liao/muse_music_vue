...
export default {
  ...
  mounted() {
    ...
    this.SlfolList = localStorage.getItem("sl_fol").split(",");
    //追蹤歌單處理
  },
  methods: {
    //最初判斷追蹤按鈕顯示
    BtnShow() {
      if (this.functype == 0) {
        if (this.SlfolList.includes(this.folnum)) {
          this.isFol = true;
        } else {
          this.isFol = false;
        }
          ...
          fetch(url, {
            ...
            .then((data) => {
              if (data.error) {
                ...
                let existingValue = localStorage.getItem("sl_fol");
                let updatedValue;
                if (this.isFol) {
                  // 将值拆分为数组，以逗号作为分隔符
                  ...
                  localStorage.setItem("sl_fol", updatedValue);
