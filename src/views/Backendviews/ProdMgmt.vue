<template>
  <div class="prodmgmt_box">
    ...
    <!-- 以下是表格資料區 -->
    <Table highlight-row stripe class="prodmgmt_table cellHeight" width="1000" height="600" :columns="columns" :data="productData">
      <!-- 上下架按鈕 -->
  ...
  <div v-if="editBox == true" class="editprod obj_Radius">
    ...
    <input type="text" id="prod_id" name="id" v-model="editItem.prod_id" style="margin-bottom: 10px; width: 300px"
      readonly />
    ...
    <input type="text" id="prod_date" name="name" v-model="editItem.prod_date" style="margin-bottom: 10px; width: 300px"
      readonly />
    ...
    <input type="text" id="prod_type" name="name" v-model="editItem.prod_type"
      ...
      style="margin-bottom: 10px; width: 300px; height: 80px"></textarea>
      <p>商品介紹:</p>
      <textarea id="prod_int" name="name" v-model="editItem.prod_int"
      style="margin-bottom: 10px; width: 300px; height: 80px"></textarea>
      <label for="show_stat" class="label state">預設狀態:</label>
      <select id="show_stat" style="margin-bottom: 10px; width: 300px" v-model="editItem.show_stat">
  ...
  <div v-if="addBox == true" class="addprod obj_Radius">
    ...
    <textarea id="addprod_int" name="name" v-model="addItem.addprod_int"
      style="margin-bottom: 10px; width: 300px; height: 80px"></textarea>
    <label for="addshow_stat" class="label state">預設狀態:</label>
    <select id="addshow_stat" style="margin-bottom: 10px; width: 300px" v-model="addItem.addshow_stat">
      <option value="1">公開</option>
      <option value="0">私人</option>
    </select>
    <p>圖片:</p>
