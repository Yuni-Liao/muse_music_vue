<template>
    <div class="robotmgmt_box">
        <h1>網站資訊管理 | 機器人管理</h1>
        <Button type="primary" @click="addBtn(row)" style="width:100px; margin-bottom: 2%; margin-left: 75%;">新增問題</Button>
        <Table highlight-row stripe class="robotmgmt_table cellHeight" width="1150" height="500" :columns="columns"
            :data="faqData">
    ...
    <div v-if="editBox == true" class="editfaq obj_Radius">
        <p>編號：</p>
        <input type="text" id="faq_id" name="name" v-model="editItem.faq_id" class="obj_Radius readonly"
            style="margin-bottom:10px; width: 360px;" readonly>
    ...
    <div v-if="addBox == true" class="addfaq obj_Radius">
        ...
        <p>回答：</p><input type="text" id="add_ans" name="name" v-model="addItem.add_ans" class="obj_Radius"
            style="margin-bottom:10px; width: 360px;">
        <p>短回答（機器人使用）：</p><input type="text" id="add_short_ans" name="name" v-model="addItem.add_short_ans"
            class="obj_Radius" style="margin-bottom:10px; width: 360px;">
