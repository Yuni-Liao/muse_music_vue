      ...
      <swiper-slide>
        <div class="topAlbums">
          ...
          <div class="topAlbumsRanking">
            <ol>
              <li v-for="(item, index) in albumRank" :key="index">
                {{ index < 5 ? index + 1 : index - 4 + 5 }}.
                <img
                ...
                <div class="txt" @click="gotosinglealbum(item.alb_id)">
                  <h3>{{ item.alb_name }}</h3>
                  <p>
                    {{ item.mem_name
                    }}<span
                      ><br />分享次數:&nbsp;{{ item.share_num }}&nbsp;次</span
                    >
                  </p>
                </div>
              </li>
            ...
            <ol>
              <li v-for="(item, index) in SLRank" :key="index">
                ...
                <img
                  ...
                  :src="`${publicPath}dataimage/song/${item.sl_pic}`"
                  alt=""
                />

                <div class="txt">
                  <h3 @click.self.prevent="gotosonglist(item.sl_id)">
                    {{ item.sl_name }}
                  </h3>
                  <p @click="gotosinger(item.creater_id)">
                    {{ item.creater_name
                    }}<span
                      ><br />追蹤次數:&nbsp;{{ item.fol_num }}&nbsp;次</span
                    >
                  </p>
                </div>
